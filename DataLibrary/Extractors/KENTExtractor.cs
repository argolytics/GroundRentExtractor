...
public class KENTExtractor
{
    private readonly IDataContext _dataContext;
    private readonly KENTDataServiceFactory _kentDataServiceFactory;
    ...
    private readonly string County = "KENT";
    ...
    public KENTExtractor(
        IDataContext dataContext,
        KENTDataServiceFactory kentDataServiceFactory,
        ...
        IOptionsMonitor<WebPageStringSettings> webPageStringSettings)
    {
        ...
        _kentDataServiceFactory = kentDataServiceFactory;
    ...
    private void SetupDriver(IOptionsMonitor<DriverPathSettings> pathSettings)
    {
        try
        {
            if (!string.IsNullOrEmpty(pathSettings.CurrentValue.FirefoxKENTProfilePath) && !string.IsNullOrEmpty(pathSettings.CurrentValue.GeckoDriverPath))
            {
                FirefoxProfile firefoxProfile = new(pathSettings.CurrentValue.FirefoxKENTProfilePath);
    ...
    public async Task Extract(int remainingCount, CancellationToken cancellationToken)
    {
        ...
        {
            var kentDataService = _kentDataServiceFactory.CreateExtractorDataService(uow);
            AddressList = await kentDataService.ReadAddressTopAmountWhereIsGroundRentNull(remainingCount);
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(_webPageStringSettings.CurrentValue.KENTDropDownSelect)));
                Input.Click();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(_webPageStringSettings.CurrentValue.NoRecordsMessage))
                    {
                        ...
                        {
                            var kentDataService = _kentDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResultBool = await kentDataService.DeleteAddress(iterAddress.AccountId);
                        }
                        currentCount++;
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(_webPageStringSettings.CurrentValue.NoGroundRentMessage))
                        {
                            ...
                            {
                                var kentDataService = _kentDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResultBool = await kentDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var kentDataService = _kentDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResultBool = await kentDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            if (await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.KENTContainer))
                                            {
                                                pdfDownloadCount++;
                                            }
                                            else
