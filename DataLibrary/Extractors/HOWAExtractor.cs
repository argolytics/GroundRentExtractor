...
public class HOWAExtractor
{
    private readonly IDataContext _dataContext;
    private readonly HOWADataServiceFactory _howaDataServiceFactory;
    ...
    private readonly string County = "HOWA";
    ...
    public HOWAExtractor(
        IDataContext dataContext,
        HOWADataServiceFactory howaDataServiceFactory,
        ...
        IOptionsMonitor<WebPageStringSettings> webPageStringSettings)
    {
        ...
        _howaDataServiceFactory = howaDataServiceFactory;
    ...
    private void SetupDriver(IOptionsMonitor<DriverPathSettings> pathSettings)
    {
        try
        {
            if (!string.IsNullOrEmpty(pathSettings.CurrentValue.FirefoxHOWAProfilePath) && !string.IsNullOrEmpty(pathSettings.CurrentValue.GeckoDriverPath))
            {
                FirefoxProfile firefoxProfile = new(pathSettings.CurrentValue.FirefoxHOWAProfilePath);
    ...
    public async Task Extract(int remainingCount, CancellationToken cancellationToken)
    {
        ...
        {
            var howaDataService = _howaDataServiceFactory.CreateExtractorDataService(uow);
            AddressList = await howaDataService.ReadAddressTopAmountWhereIsGroundRentNull(remainingCount);
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(_webPageStringSettings.CurrentValue.HOWADropDownSelect)));
                Input.Click();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(_webPageStringSettings.CurrentValue.NoRecordsMessage))
                    {
                        ...
                        {
                            var howaDataService = _howaDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResultBool = await howaDataService.DeleteAddress(iterAddress.AccountId);
                        }
                        currentCount++;
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(_webPageStringSettings.CurrentValue.NoGroundRentMessage))
                        {
                            ...
                            {
                                var howaDataService = _howaDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResultBool = await howaDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var howaDataService = _howaDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResultBool = await howaDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            if (await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.HOWAContainer))
                                            {
                                                pdfDownloadCount++;
                                            }
                                            else
