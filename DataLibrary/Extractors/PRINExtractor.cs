...
public class PRINExtractor
{
    private readonly IDataContext _dataContext;
    private readonly PRINDataServiceFactory _prinDataServiceFactory;
    ...
    private readonly string County = "PRIN";
    ...
    public PRINExtractor(
        IDataContext dataContext,
        PRINDataServiceFactory prinDataServiceFactory,
        ...
        IOptionsMonitor<WebPageStringSettings> webPageStringSettings)
    {
        ...
        _prinDataServiceFactory = prinDataServiceFactory;
    ...
    private void SetupDriver(IOptionsMonitor<DriverPathSettings> pathSettings)
    {
        try
        {
            if (!string.IsNullOrEmpty(pathSettings.CurrentValue.FirefoxPRINProfilePath) && !string.IsNullOrEmpty(pathSettings.CurrentValue.GeckoDriverPath))
            {
                FirefoxProfile firefoxProfile = new(pathSettings.CurrentValue.FirefoxPRINProfilePath);
    ...
    public async Task Extract(int remainingCount, CancellationToken cancellationToken)
    {
        ...
        {
            var prinDataService = _prinDataServiceFactory.CreateExtractorDataService(uow);
            AddressList = await prinDataService.ReadAddressTopAmountWhereIsGroundRentNull(remainingCount);
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(_webPageStringSettings.CurrentValue.PRINDropDownSelect)));
                Input.Click();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(_webPageStringSettings.CurrentValue.NoRecordsMessage))
                    {
                        ...
                        {
                            var prinDataService = _prinDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResultBool = await prinDataService.DeleteAddress(iterAddress.AccountId);
                        }
                        currentCount++;
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(_webPageStringSettings.CurrentValue.NoGroundRentMessage))
                        {
                            ...
                            {
                                var prinDataService = _prinDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResultBool = await prinDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var prinDataService = _prinDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResultBool = await prinDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            if (await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.PRINContainer))
                                            {
                                                pdfDownloadCount++;
                                            }
                                            else
