...
public class BACOExtractor
{
    private readonly IDataContext _dataContext;
    private readonly BACODataServiceFactory _bacoDataServiceFactory;
    ...
    private readonly string County = "BACO";
    ...
    public BACOExtractor(
        IDataContext dataContext,
        BACODataServiceFactory bacoDataServiceFactory,
        ...
        IOptionsMonitor<WebPageStringSettings> webPageStringSettings)
    {
        ...
        _bacoDataServiceFactory = bacoDataServiceFactory;
    ...
    private void SetupDriver(IOptionsMonitor<DriverPathSettings> pathSettings)
    {
        try
        {
            if (!string.IsNullOrEmpty(pathSettings.CurrentValue.FirefoxBACOProfilePath) && !string.IsNullOrEmpty(pathSettings.CurrentValue.GeckoDriverPath))
            {
                FirefoxProfile firefoxProfile = new(pathSettings.CurrentValue.FirefoxBACOProfilePath);
    ...
    public async Task Extract(int remainingCount, CancellationToken cancellationToken)
    {
        ...
        {
            var bacoDataService = _bacoDataServiceFactory.CreateExtractorDataService(uow);
            AddressList = await bacoDataService.ReadAddressTopAmountWhereIsGroundRentNull(remainingCount);
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(_webPageStringSettings.CurrentValue.BACODropDownSelect)));
                Input.Click();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(_webPageStringSettings.CurrentValue.NoRecordsMessage))
                    {
                        ...
                        {
                            var bacoDataService = _bacoDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResultBool = await bacoDataService.DeleteAddress(iterAddress.AccountId);
                        }
                        currentCount++;
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(_webPageStringSettings.CurrentValue.NoGroundRentMessage))
                        {
                            ...
                            {
                                var bacoDataService = _bacoDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResultBool = await bacoDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var bacoDataService = _bacoDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResultBool = await bacoDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            if (await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.BACOContainer))
                                            {
                                                pdfDownloadCount++;
                                            }
                                            else
