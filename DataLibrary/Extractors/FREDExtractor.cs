...
public class FREDExtractor
{
    private readonly IDataContext _dataContext;
    private readonly FREDDataServiceFactory _fredDataServiceFactory;
    ...
    private readonly string County = "FRED";
    ...
    public FREDExtractor(
        IDataContext dataContext,
        FREDDataServiceFactory fredDataServiceFactory,
        ...
        IOptionsMonitor<WebPageStringSettings> webPageStringSettings)
    {
        ...
        _fredDataServiceFactory = fredDataServiceFactory;
    ...
    private void SetupDriver(IOptionsMonitor<DriverPathSettings> pathSettings)
    {
        try
        {
            if (!string.IsNullOrEmpty(pathSettings.CurrentValue.FirefoxFREDProfilePath) && !string.IsNullOrEmpty(pathSettings.CurrentValue.GeckoDriverPath))
            {
                FirefoxProfile firefoxProfile = new(pathSettings.CurrentValue.FirefoxFREDProfilePath);
    ...
    public async Task Extract(int remainingCount, CancellationToken cancellationToken)
    {
        ...
        {
            var fredDataService = _fredDataServiceFactory.CreateExtractorDataService(uow);
            AddressList = await fredDataService.ReadAddressTopAmountWhereIsGroundRentNull(remainingCount);
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(_webPageStringSettings.CurrentValue.FREDDropDownSelect)));
                Input.Click();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(_webPageStringSettings.CurrentValue.NoRecordsMessage))
                    {
                        ...
                        {
                            var fredDataService = _fredDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResultBool = await fredDataService.DeleteAddress(iterAddress.AccountId);
                        }
                        currentCount++;
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(_webPageStringSettings.CurrentValue.GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(_webPageStringSettings.CurrentValue.NoGroundRentMessage))
                        {
                            ...
                            {
                                var fredDataService = _fredDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResultBool = await fredDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var fredDataService = _fredDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResultBool = await fredDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            if (await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.FREDContainer))
                                            {
                                                pdfDownloadCount++;
                                            }
                                            else
