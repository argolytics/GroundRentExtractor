...
public class CARRExtractor
{
    ...
    private readonly string CARRDropDownSelect = "#cphMainContentArea_ucSearchType_wzrdRealPropertySearch_ucSearchType_ddlCounty > option:nth-child(8)";
    ...
    public async Task Extract(int amountToExtract, CancellationToken cancellationToken)
    {
        ...
        using (var uow = _dataContext.CreateUnitOfWork())
        {
            ...
            AddressList = await carrDataService.ReadAddressTopAmountWhereIsGroundRentNull(amountToExtract);
        }
        if (AddressList.Count == 0)
        {
            foreach (string window in FirefoxDriver.WindowHandles) FirefoxDriver.Close();
            FirefoxDriver.Quit();
            Serilog.Log.Information("CARR complete.");
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                iterAddress.County = "CARR";
                // Selecting appropriate county
                FirefoxDriver.Navigate().GoToUrl(BaseUrl);
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(CARRDropDownSelect)));
                Input.Click();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(NoRecordsMessage))
                    {
                        ...
                        {
                            var carrDataService = _carrDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResult = await carrDataService.DeleteAddress(iterAddress.AccountId);
                        }
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(NoGroundRentMessage))
                        {
                            ...
                            {
                                var carrDataService = _carrDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResult = await carrDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var carrDataService = _carrDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResult = await carrDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            dbTransactionResult = await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.CARRContainer);
