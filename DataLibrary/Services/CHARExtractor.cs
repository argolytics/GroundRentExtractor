...
public class CHARExtractor
{
    ...
    private readonly CHARDataServiceFactory _dorcDataServiceFactory;
    ...
    private readonly string CHARDropDownSelect = "#cphMainContentArea_ucSearchType_wzrdRealPropertySearch_ucSearchType_ddlCounty > option:nth-child(10)";
    ...
    public CHARExtractor(
        ...
        CHARDataServiceFactory dorcDataServiceFactory,
    ...
    public async Task Extract(int amountToExtract, CancellationToken cancellationToken)
    {
        ...
        if (AddressList.Count == 0)
        {
            ...
            FirefoxDriver.Quit();
            Serilog.Log.Information("CHAR complete.");
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                iterAddress.County = "CHAR";
                // Selecting appropriate county
                FirefoxDriver.Navigate().GoToUrl(BaseUrl);
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(CHARDropDownSelect)));
                Input.Click();
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            dbTransactionResult = await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.CHARContainer);
