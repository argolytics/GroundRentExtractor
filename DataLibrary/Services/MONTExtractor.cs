...
public class MONTExtractor
{
    ...
    private readonly string MONTDropDownSelect = "#cphMainContentArea_ucSearchType_wzrdRealPropertySearch_ucSearchType_ddlCounty > option:nth-child(17)";
    ...
    public async Task Extract(int amountToExtract, CancellationToken cancellationToken)
    {
        ...
        using (var uow = _dataContext.CreateUnitOfWork())
        {
            ...
            AddressList = await montDataService.ReadAddressTopAmountWhereIsGroundRentNull(amountToExtract);
        }
        if (AddressList.Count == 0)
        {
            foreach (string window in FirefoxDriver.WindowHandles) FirefoxDriver.Close();
            FirefoxDriver.Quit();
            Serilog.Log.Information("MONT complete.");
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                iterAddress.County = "MONT";
                // Selecting appropriate county
                FirefoxDriver.Navigate().GoToUrl(BaseUrl);
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(MONTDropDownSelect)));
                Input.Click();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(NoRecordsMessage))
                    {
                        ...
                        {
                            var montDataService = _montDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResult = await montDataService.DeleteAddress(iterAddress.AccountId);
                        }
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(NoGroundRentMessage))
                        {
                            ...
                            {
                                var montDataService = _montDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResult = await montDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var montDataService = _montDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResult = await montDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            dbTransactionResult = await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.MONTContainer);
