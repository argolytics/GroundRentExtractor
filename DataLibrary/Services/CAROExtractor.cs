...
public class CAROExtractor
{
    ...
    private readonly string CARODropDownSelect = "#cphMainContentArea_ucSearchType_wzrdRealPropertySearch_ucSearchType_ddlCounty > option:nth-child(7)";
    ...
    public async Task Extract(int amountToExtract, CancellationToken cancellationToken)
    {
        ...
        using (var uow = _dataContext.CreateUnitOfWork())
        {
            ...
            AddressList = await caroDataService.ReadAddressTopAmountWhereIsGroundRentNull(amountToExtract);
        }
        if (AddressList.Count == 0)
        {
            foreach (string window in FirefoxDriver.WindowHandles) FirefoxDriver.Close();
            FirefoxDriver.Quit();
            Serilog.Log.Information("CARO complete.");
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                iterAddress.County = "CARO";
                // Selecting appropriate county
                FirefoxDriver.Navigate().GoToUrl(BaseUrl);
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(CARODropDownSelect)));
                Input.Click();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(NoRecordsMessage))
                    {
                        ...
                        {
                            var caroDataService = _caroDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResult = await caroDataService.DeleteAddress(iterAddress.AccountId);
                        }
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(NoGroundRentMessage))
                        {
                            ...
                            {
                                var caroDataService = _caroDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResult = await caroDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var caroDataService = _caroDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResult = await caroDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            dbTransactionResult = await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.CAROContainer);
