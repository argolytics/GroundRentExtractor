...
public class ANNEExtractor
{
    private readonly IDataContext _dataContext;
    private readonly BlobService _blobService;
    private readonly IOptionsMonitor<BlobSettings> _blobSettings;
    private readonly ANNEDataServiceFactory _anneDataServiceFactory;
    ...
    private readonly string ANNEDropDownSelect = "#cphMainContentArea_ucSearchType_wzrdRealPropertySearch_ucSearchType_ddlCounty > option:nth-child(3)";
    ...
    private readonly string SubdivisionInput = "#cphMainContentArea_ucSearchType_wzrdRealPropertySearch_ucEnterData_txtSubDiv";
    ...
    public ANNEExtractor(
        IDataContext dataContext,
        BlobService blobService,
        ANNEDataServiceFactory anneDataServiceFactory,
        ...
        IOptionsMonitor<BlobSettings> blobSettings)
    {
        ...
        _anneDataServiceFactory = anneDataServiceFactory;
    ...
    public async Task Extract(int amountToExtract, CancellationToken cancellationToken)
    {
        ...
        {
            var anneDataService = _anneDataServiceFactory.CreateExtractorDataService(uow);
            AddressList = await anneDataService.ReadAddressTopAmountWhereIsGroundRentNull(amountToExtract);
        }
        if (AddressList.Count == 0)
        {
            foreach (string window in FirefoxDriver.WindowHandles) FirefoxDriver.Close();
            FirefoxDriver.Quit();
            Serilog.Log.Information("ANNE complete.");
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(ANNEDropDownSelect)));
                Input.Click();
                ...
                if (iterAddress.County == "ANNE")
                {
                    ...
                    Input = WebDriverWait.Until(ExpectedConditions.ElementExists(By.CssSelector(SubdivisionInput)));
                    Input.Clear();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(NoRecordsMessage))
                    {
                        ...
                        {
                            var anneDataService = _anneDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResult = await anneDataService.DeleteAddress(iterAddress.AccountId);
                        }
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(NoGroundRentMessage))
                        {
                            ...
                            {
                                var anneDataService = _anneDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResult = await anneDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var anneDataService = _anneDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResult = await anneDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            dbTransactionResult = await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.ANNEContainer);
