...
public class DORCExtractor
{
    ...
    private readonly string DORCDropDownSelect = "#cphMainContentArea_ucSearchType_wzrdRealPropertySearch_ucSearchType_ddlCounty > option:nth-child(11)";
    ...
    public async Task Extract(int amountToExtract, CancellationToken cancellationToken)
    {
        ...
        using (var uow = _dataContext.CreateUnitOfWork())
        {
            ...
            AddressList = await dorcDataService.ReadAddressTopAmountWhereIsGroundRentNull(amountToExtract);
        }
        if (AddressList.Count == 0)
        {
            foreach (string window in FirefoxDriver.WindowHandles) FirefoxDriver.Close();
            FirefoxDriver.Quit();
            Serilog.Log.Information("DORC complete.");
        }
        ...
        try
        {
            ...
            foreach (var iterAddress in iterList)
            {
                ...
                iterAddress.County = "DORC";
                // Selecting appropriate county
                FirefoxDriver.Navigate().GoToUrl(BaseUrl);
                Input = WebDriverWait.Until(ExpectedConditions.ElementToBeClickable(By.CssSelector(DORCDropDownSelect)));
                Input.Click();
                ...
                if (FirefoxDriver.FindElements(By.CssSelector(GenericErrorTag)).Count != 0)
                {
                        ...
                        .Text.Contains(NoRecordsMessage))
                    {
                        ...
                        {
                            var dorcDataService = _dorcDataServiceFactory.CreateExtractorDataService(uow);
                            dbTransactionResult = await dorcDataService.DeleteAddress(iterAddress.AccountId);
                        }
                ...
                else
                {
                    ...
                    if (FirefoxDriver.FindElements(By.CssSelector(GroundRentErrorTag)).Count != 0)
                    {
                            ...
                            .Text.Contains(NoGroundRentMessage))
                        {
                            ...
                            {
                                var dorcDataService = _dorcDataServiceFactory.CreateExtractorDataService(uow);
                                dbTransactionResult = await dorcDataService.UpdateAddress(new AddressModel()
                                {
                    ...
                    else
                    {
                        ...
                        for (metadataCollectionCurrentCount = 0; metadataCollectionCurrentCount < metadataCollectionTotalCount;)
                        {
                            ...
                            }
                            {
                                ...
                                {
                                    var dorcDataService = _dorcDataServiceFactory.CreateExtractorDataService(uow);
                                    dbTransactionResult = await dorcDataService.CreateGroundRentPdf(new GroundRentPdfModel()
                                    {
                                ...
                                foreach (string window in FirefoxDriver.WindowHandles)
                                {
                                    if (baseUrlWindow != window)
                                    {
                                        ...
                                        if (WebDriverWait.Until(FirefoxDriver => ((IJavaScriptExecutor)FirefoxDriver).ExecuteScript("return document.readyState").Equals("complete")))
                                        {
                                            ...
                                            dbTransactionResult = await _blobService.UploadBlob(pdfFileName, printDocument, _blobSettings.CurrentValue.DORCContainer);
