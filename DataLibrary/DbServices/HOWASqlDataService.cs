...
public class HOWASqlDataService : IExtractorDataService
{
    ...
    private const string _UpdateAddressSql = @"UPDATE dbo.[HOWA] SET [AccountId] = @AccountId, [County] = @County, [AccountNumber] = @AccountNumber, [Ward] = @Ward, [Section] = @Section, [Block] = @Block, [Lot] = @Lot, [LandUseCode] = @LandUseCode, [YearBuilt] = @YearBuilt, [IsGroundRent] = @IsGroundRent, [IsRedeemed] = @IsRedeemed, [PdfCount] = @PdfCount, [AllDataDownloaded] = @AllDataDownloaded WHERE [AccountId] = @AccountId";
    private const string _DeleteAddressSql = @"DELETE FROM dbo.[HOWA] WHERE AccountId = @AccountId;";
    private const string _SelectAddressGroundRentNull = @"SELECT TOP (@Amount) [Id], [AccountId], [County], [AccountNumber], [Ward], [Section], [Block], [Lot], [LandUseCode], [YearBuilt], [IsGroundRent], [IsRedeemed], [PdfCount], [AllDataDownloaded] FROM dbo.[HOWA] where [IsGroundRent] is null";
    private const string _SelectAddressGroundRentTrue = @"SELECT TOP (@Amount) [Id], [AccountId], [County], [AccountNumber], [Ward], [Section], [Block], [Lot], [LandUseCode], [YearBuilt], [IsGroundRent], [IsRedeemed], [PdfCount], [AllDataDownloaded] FROM dbo.[HOWA] where [IsGroundRent] = 1";
    private const string _CreateGroundRentPdfSql = @"INSERT INTO dbo.[HOWAGroundRentPdf] ([AddressId], [AccountId], [DocumentFiledType], [AcknowledgementNumber], [DateTimeFiled], [PdfPageCount], [Book], [Page], [ClerkInitials], [YearRecorded]) VALUES (@AddressId, @AccountId, @DocumentFiledType, @AcknowledgementNumber, @DateTimeFiled, @PdfPageCount, @Book, @Page, @ClerkInitials, @YearRecorded); SELECT CAST(SCOPE_IDENTITY() AS INT);";
    private const string _UpdateGroundRentPdfSql = @"UPDATE dbo.[HOWAGroundRentPdf] SET [AccountId] = @AccountId, [AddressId] = @AddressId, [DocumentFiledType] = @DocumentFiledType, [AcknowledgementNumber] = @AcknowledgementNumber, [DateTimeFiled] = @DateTimeFiled, [PdfPageCount] = @PdfPageCount, [Book] = @Book, [Page] = @Page, [ClerkInitials] = @ClerkInitials, [YearRecorded] = @YearRecorded WHERE [AccountId] = @AccountId AND [DateTimeFiled] = @DateTimeFiled";
